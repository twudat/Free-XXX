...
class streamer:
    ...
    def generic(self, url, pattern=None):
        ...
        try:
            ...
            log_utils.log('Returning %s from FREE-XXX-O-DUS Resolver' % str(u), log_utils.LOGNOTICE)
            return u
    ...
    def freeomovie(self, url):
        ...
        if selected < 0:
            ...
            try:
                ...
                xbmc.Player().play(u)
            except : dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Resolver Couldn\'t Resolve Link, Try Another[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def chaturbate(self, url):
        dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Getting Cam Now[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
        try:
            ...
            xbmc.Player ().play(source)
        except: dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Performer Is Offline[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def pandamovie(self,url):
        ...
        found = 0
        xbmc.executebuiltin("Dialog.Close(busydialog)")
        dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Checking For Links Now, Be Patient[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
        for i in data:
            url = i.a['href']
    ...
    #else: dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]No Working Links Found Sorry[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def streamingporn(self,url):
        ...
        for url,name in r:
            if not 'image' in url:
                if not 'severeporn' in name:
                    if resolveurl.HostedMediaFile(url).valid_url():
                        #dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Checking For Links Now, Be Patient[/COLOR]', xbmcgui.NOTIFICATION_INFO, 13000)
                        try:
                            found +=1
        ...
        if found >= 1:
            ...
            if selected < 0:
                ...
                url2 = srcs[selected]
                dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Getting Links Now[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def xvideos(self, url):
        try:
            ...
            source = re.findall(pattern,link,flags=re.DOTALL)[0]
            xbmc.Player ().play(source)
        except: dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Couldn\'t resolve Link[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def xnxx(self, url):
        try:
            ...
            source = re.findall(pattern,link,flags=re.DOTALL)[0]
            xbmc.Player ().play(source)
        except: dialog.notification('FREE-XXX-O-DUS', '[COLOR yellow]Video Is Offline[/COLOR]', xbmcgui.NOTIFICATION_INFO, 5000)
