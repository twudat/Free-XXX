...
download_icon = translatePath(os.path.join('special://home/addons/script.freexxxodus.artwork/resources/art/main', 'downloads.png'))
...
def find_link(url, name, iconimage, downloadableLink=False):
    ...
    if downloadableLink:
        ...
        if resolveurl.HostedMediaFile(url).valid_url(): 
            ...
            log_utils.log('%s is not a valid SMU resolvable link. Attempting to resolve by FREEXXXODUS backup resolver.' % (url), log_utils.LOGNOTICE)
            try:
                ...
                kodi.notify(msg='Something went wrong!  | %s' % str(e), duration=8000, sound=True)
                quit()
            log_utils.log('%s returned by FREE-XXX-O-DUS backup resolver.' % (u), log_utils.LOGNOTICE)
