...
scraper_id = 'script.freexxxodus.scrapers'
artwork_id = 'script.freexxxodus.artwork'
GitUrl = 'https://raw.githubusercontent.com/twudat/free-xxx-o-dus/main/addons.xml'
...
def mainMenu():
    ...
    c = []
    c += [
        (kodi.giveColor('Welcome to FREE-XXX-O-DUS Version %s' % kodi.get_version(), 'blue', True), translatePath(os.path.join(kodi.addonfolder,'resources/files/information.txt')), 17, 'icon', 'Original Code by EchoCoder, Updates by @Nemzzy668, Freedom by Twudat', False),

        ('[COLOR yellow]View Changelog[/COLOR]', translatePath(os.path.join(kodi.addonfolder,
         'changelog.txt')), 17, 'changelog', 'View FREE-XXX-O-DUS Changelog.', False),
        ('[COLOR orange]Check FREE-XXX-O-DUS Health', None, 46, 'icon', 'Versions', True), \
        # ('[COLOR magenta]Install CumWithMe ( New Addon )',None,50,'icon','Versions',False),
        ('Search...', None, 29, 'search', 'Search FREE-XXX-O-DUS', True),
        ...
        # ('View Disclaimer',xbmc.translatePath(os.path.join(kodi.addonfolder, 'resources/files/disclaimer.txt')),17,'disclaimer','View FREE-XXX-O-DUS Disclaimer.',False), \
        # ('View Addon Information',xbmc.translatePath(os.path.join(kodi.addonfolder, 'resources/files/information.txt')),17,'addon_info','View FREE-XXX-O-DUS Information.',False), \
        # ('Debug Versions',None,45,'addon_info','View the versions of FREEXXXODUS and its dependencies for debugging.',True),
        ('RESET FREE-XXX-O-DUS', None, 18, 'reset',
         'Reset FREE-XXX-O-DUS to Factory Settings.', False),
...
def versioncheck():
    ...
    checkcurrentaddon = re.findall(
        '<addon\s+id="plugin.video.free-xxx-o-dus".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]
    checkcurrentscraper = re.findall(
        '<addon\s+id="script.freexxxodus.scrapers".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]
    checkcurrentartwork = re.findall(
        '<addon\s+id="script.freexxxodus.artwork".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]

    messagestring = ''
    if b == checkcurrentaddon:
        string = ('[COLOR lime]FREE-XXX-O-DUS Current : %s | Your Version : %s | All Up To Date[/COLOR]\n' %
                  (checkcurrentaddon, b))
    elif b < checkcurrentaddon:
        string = ('[COLOR orange]FREE-XXX-O-DUS Current : %s | Your Version : %s | Addon Out Of Date[/COLOR]\n' %
                  (checkcurrentaddon, b))
    elif b > checkcurrentaddon:
        string = ('[COLOR red][B]FREE-XXX-O-DUS Current : %s | Your Version : %s | Malicous Version Detected - Recommended You Un-install[/B][/COLOR]\n' % (checkcurrentaddon, b))
...
def CheckParent():
    ...
    #     dialog.ok(AddonTitle, "[COLOR aqua]Please visit [COLOR pink]https://pinsystem.co.uk[COLOR aqua] to generate an pin code for [COLOR pink]FREE-XXX-O-DUS[COLOR aqua] Addon then enter it after clicking ok[/COLOR]")
    #     string = ''
    ...
    #             try:
    #                 requests.get(
    #                     'https://pinsystem.co.uk/checker.php?code=99999&plugin=FREEXXXODUS').content
