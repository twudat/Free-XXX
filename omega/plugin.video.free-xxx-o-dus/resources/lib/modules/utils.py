...
def buildDir(items, content='dirs', cm=None, search=False, stopend=False, isVideo = False, isDownloadable = False, cache=True, chaturbate=False, pictures=False):
    ...
    for i in items:
        ...
        try:
            ...
            open_set = '%s?mode=%s' \
            % (sysaddon,str('19'))
            stext = "Open FREE-XXX-O-DUS Settings"
...
def hard_reset():
    ...
    viewDialog(translatePath(os.path.join(kodi.addonfolder , 'resources/files/reset.txt')))
    choice = xbmcgui.Dialog().yesno("[COLOR orangered][B]RESET FREE-XXX-O-DUS?[/B][/COLOR]", '[COLOR white]ARE YOU SURE YOU WANT TO RETURN FREE-XXX-O-DUS TO THE DEFAULT STATE AND LOSE ALL YOUR INFORMATION?[/COLOR]')
    if choice:
        try:
            ...
            quit()
        kodi.dialog.ok(kodi.get_name(), "[COLOR white]FREE-XXX-O-DUS has been reset to the factory state.[/COLOR]","[COLOR white]Press OK to continue.[/COLOR]")
