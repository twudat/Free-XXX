...
from resources.lib.pyxbmct_ import xxxtext
...
ART = translatePath(os.path.join('special://home/addons/script.freexxxodus.artwork', 'resources/pyxbmct/issues'))
...
def githubSelect(name):
    import githubissues
    githubissues.run('Colossal1/plugin.video.free-xxx-o-dus', '%s' % name)
    ...
    if len(items) < 1:
        msg_text = kodi.giveColor('No %s issues with FREE-XXX-O-DUS at this time.' % name.title(),'deeppink',True)
    else:
        msg_text = kodi.giveColor('%s Issues with FREE-XXX-O-DUS\n' % name.title(),'deeppink',True) + kodi.giveColor('Report Issues @ https://github.com/Colossal1/plugin.video.free-xxx-o-dus/issues','white',True) + '\n---------------------------------\n\n'
        for item in items:
