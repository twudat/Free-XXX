...
try:
    ...
    xbmc.log('Starting FREE-XXX-O-DUS Service', xbmc.LOGNOTICE)
    ...
    if ( chat_on_off ) != 'true':
        xbmc.log('Exiting FREE-XXX-O-DUS Service', xbmc.LOGNOTICE)
        quit()
    ...
    xbmc.log('Starting FREE-XXX-O-DUS Chaturbate Monitoring', xbmc.LOGNOTICE)

    while not xbmc.abortRequested:

        if not i:
            #xbmc.log('Getting Chaturbate Monitored Performers from FREE-XXX-O-DUS', xbmc.LOGNOTICE)
            conn = sqlite3.connect(chaturbatedb)
            ...
            for (title, link, iconimage) in e:
                if (not title in str(lst) ):
                    #xbmc.log('FREE-XXX-O-DUS: Checking %s' % title, xbmc.LOGNOTICE)
                    try:
        ...
        if i >= 2879:
            lst = []
            xbmc.log('FREE-XXX-O-DUS: Reseting Checked List', xbmc.LOGNOTICE)
        i = (i + 1) % 60
        j = (j + 1) % 2880
except: pass
